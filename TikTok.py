...
class TikTok():
    ...
    def tikTokUrlCheck(self, tiktokUrl):
        ...
        if '/video/' in realTikTokUrl:
            return {
                ...
                "nickname": re.findall('\@(.*)' + '/video/', realTikTokUrl)[0],
                "videoId": re.findall('/video/(\d+)', realTikTokUrl)[0]
            }
        else:
            return {
                "typeUrl": self.isUserPage,
                "nickname": re.findall('\@(.*)', realTikTokUrl)[0],
                "videoId": ""
            }
    ...
    def getVideoData(self, tiktokUrl, jsonOutput=True):
        videoDataInfo = []
            ...
            while not done:
                ...
                if response.status_code == 200:
                    ...
                    for videoItem in itemListData:
                        ...
                        nickname = self.getNicknameAndUrlNoWatermark(videoId)['nickname']
                        videoUrl = self.getVideoUrl(nickname, videoId)
                        videoUrlNoWatermark = self.getNicknameAndUrlNoWatermark(videoId)['url']
                        saveFolder = os.path.join(self.savePathFolder, typeUrl, nickname)
    ...
    def download(self, videoDataInfo):
        numberOfVideo = len(videoDataInfo)
        for index in range(numberOfVideo):
            ...
            try:
                if filename in saveFolderListDir:
                    print(f'>> {videoIndex:2>} / {numberOfVideo}, file [ {filename} ] was Exists, Download skip! ', end = "")
                    for i in range(15):
            ...
            for againNumber in range(retryDownloadMax):
                try:
                    print(f'>> Total: {videoIndex:>2} / {numberOfVideo}')
